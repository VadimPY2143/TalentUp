            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr,0.8fr]">
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
                ...
                <div className="mt-4 space-y-3">
                  {funnel.length === 0 && <div className="text-sm text-slate-500">Немає даних.</div>}
                  {funnel.length > 0 && (() => {
                    const top = Math.max(1, ...funnel.map((s) => s.count))
                    return funnel.map((s) => {
                      const widthPct = Math.max(2, Math.round((s.count / top) * 100))
                      const pct = funnel[0]?.count ? toPercent(s.count, funnel[0].count) : "0%"
                      return (
                      ...
                      {apps.map((row) => (
                        <tr key={row.id} className="border-b border-slate-100 last:border-b-0">
                          <td className="px-2 py-3">
                            <div className="font-semibold text-slate-800">{row.vacancy_title ?? `Vacancy #${row.vacancy_id}`}</div>
                            <div className="text-xs text-slate-500">ID: {row.vacancy_id}</div>
                          </td>
