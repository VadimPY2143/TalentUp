...
async def test_basic_security_headers_present(app):
    ...
    # Don't hard-fail the whole suite: in many projects headers are added later via
    # middleware / reverse proxy. Keep the signal visible without breaking CI.
    if missing:
        pytest.xfail(f"Missing recommended security headers: {', '.join(missing)}")
